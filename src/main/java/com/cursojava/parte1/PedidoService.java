...
public class PedidoService {
    ...
    public Optional<Pedido> buscarPorIdConFallback(Long id) {
        return buscarPorId(id).or(() -> buscarPorId(1L));
    }
    ...
    public ResultadoBusqueda buscarDetalle(Long id) {
        return buscarPorId(id).<ResultadoBusqueda>map(ResultadoExito::new).orElseGet(() -> new ResultadoError("Pedido no encontrado"));
    }
}